...
class BaseINS:
    ...
    def plot_errors(self, imu_time: np.ndarray, position_error: np.ndarray, velocity_error: np.ndarray,
        ...
        fig, axes = plt.subplots(3, 1, figsize=(12, 18))
        sns.lineplot(x=imu_time, y=orientation_error_smooth[:, 0] * koef[2][0] * 60, ax=axes[0])
        axes[0].set_title('Ошибка по углу крена (Roll)', fontsize=16)
        axes[0].set_xlabel('Время (с)', fontsize=14)
        axes[0].set_ylabel('Ошибка (минуты)', fontsize=14)
        axes[0].grid(True)

        sns.lineplot(x=imu_time, y=orientation_error_smooth[:, 1] * koef[2][1] * 60, ax=axes[1])
        axes[1].set_title('Ошибка по углу тангажа (Pitch)', fontsize=16)
        axes[1].set_xlabel('Время (с)', fontsize=14)
        axes[1].set_ylabel('Ошибка (минуты)', fontsize=14)
        axes[1].grid(True)

        sns.lineplot(x=imu_time, y=orientation_error_smooth[:, 2] * koef[2][2] * 60, ax=axes[2])
        axes[2].set_title('Ошибка по углу курсу (Yaw)', fontsize=16)
        axes[2].set_xlabel('Время (с)', fontsize=14)
        axes[2].set_ylabel('Ошибка (минуты)', fontsize=14)
        axes[2].grid(True)
